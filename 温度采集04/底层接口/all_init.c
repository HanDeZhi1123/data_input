...
void timer_irq_func(struct timer_list *timer)
{
    ...
    {
    case FAN:
        gpiod_set_value(gpiod_node[FAN], !gpiod_get_value(gpiod_node[FAN]));break;
    case MOTOR:
        gpiod_set_value(gpiod_node[MOTOR], !gpiod_get_value(gpiod_node[MOTOR]));break;
    case BUZZER:
        gpiod_set_value(gpiod_node[BUZZER], !gpiod_get_value(gpiod_node[BUZZER]));break;
    }
...
if (off_timer){
    ...
    while (Mode[2])
    {
       time_max = timer_s[0]-timer_s[1]*15;
       time_min = timer_s[0]-time_max;
        if(Mode[3]){
        pwm_timer.expires = falg_mode==0?jiffies+time_max:jiffies+time_min;     //0高电平时间、1低电平时间
